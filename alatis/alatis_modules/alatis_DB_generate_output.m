function alatis_DB_generate_output(folder_path, input_name, inchi_complete_path, pdb_output_name, xyz_output_name, Three_D_flag, updated_mol_path)
global Error_Msg

if isempty(Error_Msg)
    generate_other_FType_outputfiles(updated_mol_path, pdb_output_name, xyz_output_name);
end
write_sdf_output(folder_path, Three_D_flag, input_name, pdb_output_name, xyz_output_name);
...
delete_extra_files(folder_path)

function delete_extra_files(folder_path)
global compact_sdf_output_name
List = dir(folder_path);
for i=1:length(List)
    if ~strcmp(List(i).name, compact_sdf_output_name)
        fname = sprintf('%s/%s', folder_path, List(i).name);
        [~, ~] = system(sprintf('rm -f %s', fname));
    end
end

function compress_outputs(folder_path, input_name, pdb_output_name, xyz_output_name)
global updated_mol_name compact_sdf_output_name Split_mol_names Split_map_names 
Here = pwd;
cd(folder_path);
...
for i=1:length(Split_mol_names)
    Command= sprintf('%s %s', Command, Split_mol_names{i});
    Command= sprintf('%s %s', Command, Split_map_names{i});
end
[~, ~] = system(Command);
cd(Here);
function write_sdf_output(folder_path, Three_D_flag, input_name, pdb_output_name, xyz_output_name)
...
% write input
fprintf(fout,'>  <Input_Structure>\n');
if exist(input_name, 'file')
    filetext = fileread(input_name);
    fprintf(fout, '%s\n', strrep(filetext, '$$$$', ''));
end
fprintf(fout, '\n');
...
fprintf(fout,'>  <ALATIS_PDB_output>\n');
if exist(pdb_output_name, 'file')
    filetext = fileread(pdb_output_name);
    fprintf(fout, '%s\n', filetext);
end
fprintf(fout, '\n');

fprintf(fout,'>  <ALATIS_XYZ_output>\n');
if exist(xyz_output_name, 'file')
    filetext = fileread(xyz_output_name);
    fprintf(fout, '%s\n', filetext);
end
fprintf(fout, '\n');
